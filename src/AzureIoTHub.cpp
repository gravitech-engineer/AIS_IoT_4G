...
bool AzureIoTHub::connect() {
    ...
    */
    ...
    Serial.println(" MQTT Host: " + this->host);
...
void AzureIoTHub::loop() {
    ...
    while(xQueueReceive(commandReplyQueue, &reply, 0) == pdPASS) {
        if (reply) {
            Serial.println("RID: " + String(reply->rid) + ", Replay: " + String(reply->status));
            this->mqtt->publish(
